...
<div class="block">  
    <div class="navbar block-inner block-header">
        <div class="row">
            <p class="text-muted">Título do Exame: <?php echo (!empty($mock_title)) ? $mock_title->title_name : ''; ?>
            <?php 
            if (($mock_title->user_id == $this->session->userdata['user_id']) || ($this->session->userdata['user_role_id'] < 3)) {
            ?>
                <a class="btn custom_navbar-btn btn-primary pull-right col-sm-3" href="<?php echo base_url('index.php/admin_control/add_question') . '/' . $mock_title->title_id; ?>"><i class="glyphicon glyphicon-plus-sign"></i>&nbsp; Adicionar pergunta</a>
            <?php 
            } ?>
            </p>
        </div>
        ...
        <tr class="accordion-group <?=($i & 1) ? 'even' : 'odd'; ?>">
            ...
            <td class="accordion-heading">
                ...
                <div class="accordion-body collapse" id="collapse_<?php echo $i; ?>">
                    ...
                    <p><span class="text-muted"> Tipo de opção: </span><?=$mock->option_type ?> 
                    <?php if ($mock->option_type == 'Radio') { ?>
                        <span class="pull-right"> <i class="glyphicon glyphicon-warning-sign"></i> O rádio não pode ter mais de uma resposta certa.</span> <br/>
                    <?php } ?>
                    </p>
            ...
            <td class="col-xs-3">
                <div class="btn-group">
                    <a href="#collapse_<?=$i; ?>"  data-toggle="collapse" class="btn btn-sm btn-default accordion-toggle "><i class="glyphicon glyphicon-resize-small"></i><span class="invisible-on-sm"> Visão</span></a>
                    <a class="btn btn-sm btn-default update"  data-update="<?=$mock->ques_id;?>" href="#update_ques" data-toggle="modal"><i class="glyphicon glyphicon-edit"></i><span class="invisible-on-md">  Modificar</span></a>
                    <a onclick="return delete_confirmation()" href = "<?php echo base_url('index.php/admin_control/delete_question/' . $mock->ques_id); ?>" class="btn btn-sm btn-default"><i class="glyphicon glyphicon-trash"></i><span class="invisible-on-md">  Excluir</span></a>
                </div>
            </td>
        </tr>
        <?php
    ...
    <?php
        ...
        echo 'Esta simulação não tem dúvidas!';
    }
    ?>
    </div>
    </div>
