...
<div class="block">  
    <div class="navbar block-inner block-header">
        <div class="row">
            <p class="text-muted">Título do curso: <?=$courses->course_title; ?> 
            <?php 
            if ($courses->user_id == $this->session->userdata['user_id']) {
            ?>
                <a class="btn custom_navbar-btn btn-info pull-right col-sm-2" href="<?=base_url('index.php/course/add_section/'.$courses->course_id); ?>" data-toggle="modal"><i class="glyphicon glyphicon-plus-sign"></i>&nbsp; Add seção</a>
                <a class="btn custom_navbar-btn btn-primary pull-right col-sm-2" href="<?=base_url('index.php/course/add_content/'.$courses->course_id); ?>"><i class="glyphicon glyphicon-plus-sign"></i>&nbsp; Add Conteúdo</a>
            <?php 
            } ?>
            </p>
        </div>
    ...
    <div class="col-sm-12">
            ...
            <ul class="inner-sortlist">
                ...
                <li class="pull-right">
                    <div class="btn-group pull-right">
                        <a href="<?= base_url('index.php/course/section_detail/' . $section->section_id); ?>"  class="btn btn-xs btn-success "><i class="glyphicon glyphicon-resize-small"></i><span class="invisible-on-sm"> Visão</span></a>
                        <a class="btn btn-xs btn-primary update"  data-update="<?=$section->section_id;?>" href="#update_section" data-toggle="modal"><i class="glyphicon glyphicon-edit"></i><span class="invisible-on-md">  Modificar</span></a>
                        <a onclick="return delete_confirmation()" href = "<?=base_url('index.php/course/delete_section/' . $section->section_id); ?>" class="btn btn-xs btn-danger"><i class="glyphicon glyphicon-trash"></i><span class="invisible-on-md">  Excluir</span></a>
                    </div>
                </li>                                
            </ul>
        </li>
        ...
        <a id="button" class="btn btn-sm btn-warning">Atualizar pedidos</a> <i class="fa fa-warning"></i> Você pode reordenar a exibição arrastando a lista.
    </div>
    </div>
    </div>
</div><!--/span-->
...
<div class="modal fade" id="update_section" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="TRUE">  
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Seção de atualização</h4>
      </div>
      <div class="modal-body">
        <?php echo form_open(base_url() . 'index.php/course/update_section','role="form" class="form-horizontal"'); ?>
          ...
          <div class="form-group">
            <label for="section_name" class="col-xs-3 control-label">Nome da Seção :</label>
            <div class="col-xs-8">
                <?php echo form_input('section_name', '', 'id="update_section_name" placeholder="Section name" class="form-control" required="required"') ?>
            </div>
          </div>
          <div class="form-group">
            <label for="section_title" class="col-xs-3 control-label">Título da seção :</label>
            <div class="col-xs-8">
