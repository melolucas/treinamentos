...
class Exam_control extends MS_Controller
{
    ...
    public function start_exam($id = '', $message = '')
    {
        ...
        $data['message'] = $message;
        $data['mock'] = $this->exam_model->get_mock_by_id($id);
        $data['exames'] = $this->exam_model->get_exame($id);
  ...
  //     var_dump($user_info->user_id);die;
      ...
      $usuario = $user_info->user_id;
       // var_dump($userId);die;
       $dt_exame =  $data['exames'];
   
      // $data_nova = $timestamp[0] =date('Y-m-d h:s:m', strtotime($dt_exame));

      //$result = array();
      foreach ($dt_exame as $dt) {
      $result = $dt->exam_taken_date;
      

      }
    //  var_dump($id);die;

      $dataExameU = $this->exam_model->get_examePeriodo($usuario,$id);

      foreach ($dataExameU as $dtE) {
        $resultData = $dtE->exam_taken_date;
   ...
   //  var_dump($resultData);die;
    ...
    if(!empty($resultData)){
     //  var_dump('aqui') ;die;
      $data_liberacao = date('Y-m-d',strtotime('+3 days', strtotime($resultData)));
      $data_fazer = date('d/m/Y', strtotime($data_liberacao));
    }
      ...
      //  var_dump($data['mock']);die;
     

    if($data_hj > $data_fazer || ($data_fazer == '')  ){
        ...
        $this->session->unset_userdata('payment_token');
            ...
            $message = '<div class="alert alert-danger alert-dismissable">'
            . '<button type="button" class="close" data-dismiss="alert" aria-hidden="TRUE">&times;</button>'
            . 'Você só pode refazer esse exame no dia '. $data_fazer.' novamente!</div>';
        $this->session->set_flashdata('message', $message);
