...
class Login_control extends MS_Controller
{
               ...
               // echo "cheguei";exit;
                ...
                if ($this->login_model->login_check()) {
                 ...
                 redirect('');
                    ...
                    $message = '<div class="alert alert-danger">Esse Email/Senha esta incorreto!</div>';
                }
            }
        }
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() != FALSE) {
                ...
                if ($this->login_model->register($info)) {
                    ...
                    $to = $info['user_email'];
                    $suject = 'Obrigado por se cadastrar no ' . $this->session->userdata['brand_name'];
                    $message_body = 'Click no Link para ativar sua conta no EAD Maxxi.<br/> '
                            . anchor(base_url('index.php/login_control/activate/') . '?user=' . $info['user_email'] . '&key=' . $key, 'Link de Ativação');
                    ...
                    $message = '<div class="alert alert-success alert-dismissable">'
                            . '<button type="button" class="close" data-dismiss="alert" aria-hidden="TRUE">&times;</button>'
                            . 'Seu Registro foi Criado com Sucesso! Verifique seu email para ativar a conta.'
                            . '</div>';
                    if (count($_POST) > 0) {
                        $_POST = array();
                    }
                ...
                redirect(base_url('index.php/login_control'));
                    ...
                    $message= '<div class="alert alert-danger alert-dismissable">'
                            . '<button type="button" class="close" data-dismiss="alert" aria-hidden="TRUE">&times;</button>'
                            . '"' . $info['user_email'] . '" esse email esta sendo usado por outra conta. Tente outro email.</div>';
                }
            }
        }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->user_model->check_user_exist($email)) {
                ...
                if ($this->email->send()) {
                    ...
                    $message = '<div class="alert alert-success alert-dismissable">'
                            . '<button type="button" class="close" data-dismiss="alert" aria-hidden="TRUE">&times;</button>'
                            . 'Senha enviada para seu e-mail! Verifique sua conta.'
                            . '</div>';
                    $this->session->set_flashdata('message', $message);
                    redirect(base_url('index.php/login_control'));
                } else {
            ...
            if ($key == $this->input->post('key')) {
                ...
                if ($this->user_model->reset_password($this->input->post('mail'))) {
                    ...
                    $message_body = 'You\'ve successfully changed your ' . $this->session->userdata['brand_name'] . ' password.'
                            . '.<br/> Você não tem acesso. Conta Desativada'
        ...
        if ($key == $this->input->post('key')) {
            ...
            if ($this->user_model->report_password_reset($this->input->get('user'))) {
                $message = '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="TRUE">&times;</button>'
                        . 'Desativado com sucesso! Sua conta não está mais acessível. Entre em contato com o administrador.'
                        . '</div>';
                $this->session->set_flashdata('message', $message);
                redirect(base_url('index.php/login_control'));
            } else {
    ...
    public function user_role_check($val)
    {
        ...
        if ($val == 0) {
            $this->form_validation->set_message('user_role_check', 'Selecione o tipo de Usúario.');
            return FALSE;
        } else {
            return TRUE;
        }
