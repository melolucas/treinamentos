...
class Google {
    ...
    public function save_access_token($auth_code) {
        ...
        if ($new_access_token) {
            $this->ci->Settings_model->save_setting("google_drive_oauth_access_token", $new_access_token);

            //got the valid access token. store to setting that it's authorized
            $this->ci->Settings_model->save_setting("google_drive_authorized", "1");
        }
    ...
    private function _save_id($name = "", $id = "", $type = "folder", $path_type = "node") {
        if ($path_type == "parent") {
            ...
            $this->ci->Settings_model->save_setting("google_drive_parent_folder_id", $id);
        } else {
            $final_ids = array();
            if ($type == "folder") {
            ...
            $this->ci->Settings_model->save_setting($setting_name, serialize($final_ids));
        }
    }
    ...
    private function _get_id($name = "", $type = "folder") {
        if ($type == "folder") {
            $ids = unserialize(get_setting("google_drive_folder_ids"));
        } else {
            $ids = unserialize(get_setting("google_drive_temp_file_ids"));

            //for temp file id, remove old one
            if (get_array_value($ids, $name)) {
                $final_ids = $ids;
                unset($final_ids[$name]);
                $this->ci->Settings_model->save_setting("google_drive_temp_file_ids", serialize($final_ids));
            }
        }
