...
// Widget Per Capita
if (!function_exists('per_percapita_widget')) {

    function per_capita_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["per_capita"] = $ci->Funcionarios_model->infoPerCapita();
        return $ci->load->view("timeline/per_capita_widget", $view_data, $return_as_data);
    }
}

// Widget Meta de Vendas
if (!function_exists('meta_vendas_widget')) {

    function meta_vendas_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["meta_vendas"] = $ci->Metas_model->selecionarRegiao();
        return $ci->load->view("timeline/meta_vendas_widget", $view_data, $return_as_data);
    }
}

// Widget Meta % Desconto
if (!function_exists('meta_desconto_widget')) {

    function meta_desconto_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["meta_desconto"] = $ci->Metas_model->selecionarRegiao();
        return $ci->load->view("timeline/meta_desconto_widget", $view_data, $return_as_data);
    }
}

// Widget Meta de Suplementos
if (!function_exists('meta_suplementos_widget')) {

    function meta_suplementos_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["meta_suplementos"] = $ci->Metas_model->selecionarRegiao();
        return $ci->load->view("timeline/meta_suplementos_widget", $view_data, $return_as_data);
    }
}

// Widget Meta % Genéricos
if (!function_exists('meta_genericos_widget')) {

    function meta_genericos_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["meta_genericos"] = $ci->Metas_model->selecionarRegiao();
        return $ci->load->view("timeline/meta_genericos_widget", $view_data, $return_as_data);
    }
}

// Widget Metas Tícket Médio
if (!function_exists('meta_tkm_widget')) {

    function meta_tkm_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["meta_tkm"] = $ci->Metas_model->selecionarRegiao();
        return $ci->load->view("timeline/meta_tkm_widget", $view_data, $return_as_data);
    }
}

// Widget Meta de Vendas Lojas
if (!function_exists('meta_vendas_Lojas_widget')) {

    function meta_vendas_Lojas_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["meta_vendas_Lojas"] = $ci->Metas_model->selecionarRegiao();
        return $ci->load->view("timeline/meta_vendasLojas_widget", $view_data, $return_as_data);
    }
}

// Widget Meta % Desconto Lojas
if (!function_exists('meta_desconto_Lojas_widget')) {

    function meta_desconto_Lojas_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["meta_desconto_Lojas"] = $ci->Metas_model->selecionarRegiao();
        return $ci->load->view("timeline/meta_descontoLojas_widget", $view_data, $return_as_data);
    }
}

// Widget Meta de Suplementos Lojas
if (!function_exists('meta_suplementosLojas_widget')) {

    function meta_suplementos_Lojas_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["meta_suplementos_Lojas"] = $ci->Metas_model->selecionarRegiao();
        return $ci->load->view("timeline/meta_suplementosLojas_widget", $view_data, $return_as_data);
    }
}

// Widget Meta % Genéricos Lojas
if (!function_exists('meta_genericos_Lojas_widget')) {

    function meta_genericos_Lojas_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["meta_genericos_Lojas"] = $ci->Metas_model->selecionarRegiao();
        return $ci->load->view("timeline/meta_genericosLojas_widget", $view_data, $return_as_data);
    }
}

// Widget Meta Tícket Médio Lojas
if (!function_exists('meta_tkm_Lojas_widget')) {

    function meta_tkm_Lojas_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["meta_tkm_Lojas"] = $ci->Metas_model->selecionarRegiao();
        return $ci->load->view("timeline/meta_tkmLojas_widget", $view_data, $return_as_data);
    }
}
...
if (!function_exists('count_total_time_widget')) {
    ...
    function count_total_time_widget($user_id = 0, $return_as_data = false) {
        ...
        $view_data["total_hours_worked"] = number_format($info->timecard_total / 60 / 60, 2,',','.');
